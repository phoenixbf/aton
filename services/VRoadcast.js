...
VRoadcast.user = class {
    ...
    sendSnapshot(socket){
        if (this.name)     socket.emit("UNAME", { uid: this.uid, name: this.name });
        if (this.message)  socket.emit("UMSG", { uid: this.uid, msg: this.message });
        if (this.binState) socket.emit("USTATE", this.binState);
    }
};
